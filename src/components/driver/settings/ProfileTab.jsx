...
import { Camera, Save, Loader2, CheckCircle2, Circle, X } from "lucide-react";
    ...
    <div className="space-y-4">
      ...
      <Card className={allDone ? "border-lhs-green bg-lhs-green/5" : ""}>
        <CardContent className="p-4 space-y-3">
          ...
          {!allDone && <p className="text-xs text-muted-foreground">Complete these in any order.</p>}
      ...
      <Card>
        <CardContent className="p-4 space-y-3">
          ...
          <p className="text-xs text-muted-foreground">Upload a clear photo of your face. No sunglasses, no hat.</p>
      ...
      <Card>
        <CardContent className="p-4 space-y-4">
          ...
          <Field label="Username" helper="Your public handle for rankings and member content.">
          ...
          <Field label="Bio" helper={`${bio.length}/120 characters`}>
            <input value={bio} onChange={(e) => setBio(e.target.value.slice(0, 120))} placeholder="e.g. 15 years OTR, Midwest specialist"
              className="w-full border border-input rounded-lg px-3 py-2 text-sm bg-background focus:outline-none focus:ring-1 focus:ring-ring" />
          </Field>
        </CardContent>
      </Card>
      ...
      {/*
        CDL collection is deferred from the MVP.
        Future implementation will support driver verification, safety certification, and other operational workflows.
        The cdl_number and cdl_state fields remain in the data model and are still persisted on save — they are simply not exposed in this UI.
      */}
        ...
        <CardContent className="p-4 space-y-3">
          ...
          {cocAccepted && cocDate ? (
            ...
            <div className="space-y-3">
              <button onClick={() => { setCocViewed(true); setShowCoc(true); }} className="text-sm text-primary underline underline-offset-2 font-medium">
                View Code of Conduct
              </button>
              <label className={`flex items-start gap-2.5 cursor-pointer ${!cocViewed ? "opacity-40 pointer-events-none" : ""}`}>
                <input type="checkbox" checked={cocAccepted}
                  onChange={(e) => { if (e.target.checked) handleCocAccept(); else { setCocAccepted(false); setCocDate(null); } }}
                  disabled={!cocViewed} className="mt-0.5 accent-fuel-orange w-4 h-4" />
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 p-4">
          <div className="bg-background rounded-2xl w-full max-w-lg max-h-[80vh] flex flex-col">
            ...
            <div className="p-4 border-t">
              <Button className="w-full" onClick={handleCocAccept}>I Accept the Code of Conduct</Button>
            </div>
          </div>
        </div>
      )}
